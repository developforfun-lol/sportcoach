...
// IDB
let db;const req=indexedDB.open('sportcoach',12);
...
async function startRec(){
  ...
  currentMime=pickType();chunks=[];
  currentRecordingMarkers=[];recStartPerf=performance.now();
...
async function loadVideo(id){
  ...
  tx.objectStore('videos').get(id).onsuccess=async(e)=>{
    ...
    if(!url){alert('Video blob missing');return}
    currentObjectUrl=url;player.src=url;await player.play().catch(()=>{});
...
function renderMarkers(){
  ...
  currentMeta.markers.forEach(m=>{
    ...
    s.textContent=' rec '+fmtS(Math.floor(m.recMs/1000))+' | game '+fmt(Math.floor(m.gameMs));
    chip.appendChild(s);
    chip.onclick=()=>{player.currentTime=Math.max(0,m.recMs/1000-0.3)};
...
function resizeCanvas(){const r=canvas.getBoundingClientRect();canvas.width=r.width*devicePixelRatio;canvas.height=r.height*devicePixelRatio;ctx.scale(devicePixelRatio,devicePixelRatio);redraw()}
...
async function deleteVideo(id){
  await new Promise((resolve)=>{const tx=db.transaction('videos','readwrite');const store=tx.objectStore('videos');const req=store.delete(id);req.onsuccess=()=>resolve()});
...
document.getElementById('freeSpace').onclick=async()=>{
  if(!confirm('This clears ALL videos and blobs. Continue?'))return;
  await new Promise((resolve)=>{const tx=db.transaction('videos','readwrite');tx.objectStore('videos').clear();tx.oncomplete=resolve});
  const cache=await caches.open('blobs');const keys=await cache.keys();await Promise.all(keys.map(k=>cache.delete(k)));refreshLibrary();
};
